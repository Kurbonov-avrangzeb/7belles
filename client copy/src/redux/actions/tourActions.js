...
export const fetchTour = id => async dispatch => {
  const response = await api.get(`/api/v1/tours/${id}`);
...
export const createTour = (formValues) => async (dispatch) => {
  try {
    ...
    const response = await api.post('/api/v1/tours', formData);
...
export const updateTour = (id, formValues) => async dispatch => {
  ...
  const response = await api.patch(`/api/v1/tours/${id}`, formData);
